...
CREATE TABLE IF NOT EXISTS `box` (
  ...
  `characteristic` TEXT NOT NULL COMMENT '特征',
  `logistics` TEXT NOT NULL COMMENT '送货',
  `about` TEXT NOT NULL COMMENT '关于主题',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted_at` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '删除时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `order` (
  ...
  `post_phone` VARCHAR(20) NOT NULL DEFAULT '' COMMENT '邮寄电话',
  `post_addr` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '邮寄地址',
  `is_send_email` TINYINT(4) NOT NULL DEFAULT '0' COMMENT '是否已经发送邮件[0:未发送,1:已发送]',
...
CREATE TABLE IF NOT EXISTS `pay_callback_result` (
  ...
  `notify_type` TINYINT NOT NULL DEFAULT 0 COMMENT '通知类型[0:支付宝同步通知,1:支付宝异步通知,2:微信同步通知,3:微信异步通知]',
  `pay_type` TINYINT NOT NULL DEFAULT 0 COMMENT '支付类型[0:支付宝电脑网站支付,1:支付宝手机网站支付,2:微信PC支付]',
...
CREATE TABLE IF NOT EXISTS `vote` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'PK',
  `count` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '票数计数',
  `content` VARCHAR(60) NOT NULL DEFAULT '' COMMENT '投选内容',
  `image` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '图片',
  `type` tinyint(4) NOT NULL DEFAULT 0 COMMENT '类型[0:投妹子]',
  `status` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '状态[0:显示,1:不显示]',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted_at` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '删除时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 COMMENT='投票';